...
bool ConsoleGameEngine::ConstructScreen(int width, int height, int pixelWidth, int pixelHeight)
{
	if (width < 1 || height < 1)
...
int ConsoleGameEngine::Error(const wchar_t* message)
{
...
int ConsoleGameEngine::GetScreenWidth()
{
	return screenWidth;
}

int ConsoleGameEngine::GetScreenHeight()
{
	return screenHeight;
}

short ConsoleGameEngine::GetScreenCharacter(int i)
{
	return screen[i].Char.UnicodeChar;
}

short ConsoleGameEngine::GetScreenCharacter(int x, int y)
{
	return screen[screenWidth * y + x].Char.UnicodeChar;
}

short ConsoleGameEngine::GetScreenColor(int i)
{
	return screen[i].Attributes;
}

short ConsoleGameEngine::GetScreenColor(int x, int y)
{
	return screen[screenWidth * y + x].Attributes;
}

void ConsoleGameEngine::SetApplicationTitle(LPCWSTR title)
{
	SetConsoleTitle(title);
}

bool ConsoleGameEngine::OnDestroy()
{
	return true;
}
...
void Sprite::SetCharacter(int x, int y, short character)
{
	if (x < 0 || x >= width || y < 0 || y >= height) return;
	contents[width * y + x].Char.UnicodeChar = character;
}

short Sprite::GetCharacter(int x, int y)
{
	if (x < 0 || x >= width || y < 0 || y >= height) return ' ';
	return contents[width * y + x].Char.UnicodeChar;
}

const short Sprite::GetCharacter(int x, int y) const
{
	if (x < 0 || x >= width || y < 0 || y >= height) return ' ';
	return contents[width * y + x].Char.UnicodeChar;
}
...
const short Sprite::GetColor(int x, int y) const
{
	if (x < 0 || x >= width || y < 0 || y >= height) return BG_BLACK;
	return contents[width * y + x].Attributes;
}

short Sprite::SampleCharacter(float x, float y)
{
...
void ConsoleGameEngine::Draw(int index, short character, short color)
{
...
void ConsoleGameEngine::DrawPoint(int x, int y, short character, short color)
{
...
void ConsoleGameEngine::DrawLine(int x0, int y0, int x1, int y1, short character, short color)
{
	int dx = abs(x1 - x0);
	int sx = (x0 < x1) ? 1 : -1;
	int dy = -abs(y1 - y0);
...
void ConsoleGameEngine::DrawTriangle(int x0, int y0, int x1, int y1, int x2, int y2, short character, short color)
{
...
void ConsoleGameEngine::DrawFilledTriangle(int x0, int y0, int x1, int y1, int x2, int y2, short character, short color)
{
	if (y1 < y0)
	{
		std::swap(y1, y0);
...
void ConsoleGameEngine::FillBottomFlatTriangle(int x0, int y0, int x1, int y1, int x2, int y2, short character, short color)
{
...
void ConsoleGameEngine::FillTopFlatTriangle(int x0, int y0, int x1, int y1, int x2, int y2, short character, short color)
{
...
void ConsoleGameEngine::DrawRectangle(int x0, int y0, int x1, int y1, short character, short color)
{
...
void ConsoleGameEngine::DrawFilledRectangle(int x0, int y0, int x1, int y1, short character, short color)
{
	if (y1 < y0)
	{
		std::swap(y1, y0);
...
void ConsoleGameEngine::DrawCircle(int cx, int cy, int r, short character, short color)
{
	int x = r;
	int y = 0;
	int sx = 1 - (2 * r);
...
void ConsoleGameEngine::DrawFilledCircle(int cx, int cy, int r, short character, short color)
{
	int x = r;
	int y = 0;
	int sx = 1 - (2 * r);
...
void ConsoleGameEngine::DrawSpriteAlpha(int x, int y, Sprite& sprite, short transparencyCol)
{
	for (int i = 0; i < sprite.GetWidth(); i++)
	{
		for (int j = 0; j < sprite.GetHeight(); j++)
...
void ConsoleGameEngine::DisplayText(int x, int y, std::wstring text, short color)
{
	int index = screenWidth * y + x;
...
void ConsoleGameEngine::DisplayTextAlpha(int x, int y, std::wstring text, short color)
{
	int index = screenWidth * y + x;
	for (unsigned int i = 0; i < text.size(); i++)
	{
...
void ConsoleGameEngine::Fill(int x, int y, int length, short character, short color)
{
	int index = screenWidth * y + x;
...
void ConsoleGameEngine::ClearScreen()
{
	Fill(0, 0, screenWidth * screenHeight, ' ', BG_BLACK);
}

void ConsoleGameEngine::FloodFill(int x, int y, short color)
{
...
void ConsoleGameEngine::ReadInput()
{
	for (int i = 0; i < 256; i++)
	{
...
short ConsoleGameEngine::GetMouseX()
{
	return mouseX;
}

short ConsoleGameEngine::GetMouseY()
{
	return mouseY;
}

ConsoleGameEngine::KeyState ConsoleGameEngine::GetKey(short key)
{
	return keys[key];
}
...
ConsoleGameEngine::AudioClip::AudioClip() {}

ConsoleGameEngine::AudioClip::~AudioClip() {}

ConsoleGameEngine::AudioClip::AudioClip(std::wstring fileName)
{
	FILE *file = nullptr;
...
ConsoleGameEngine::CurrentlyPlayingClip::CurrentlyPlayingClip() {}

ConsoleGameEngine::CurrentlyPlayingClip::CurrentlyPlayingClip(int id, bool looped) : audioClipID(id), looped(looped) {}

void ConsoleGameEngine::CurrentlyPlayingClip::Restart()
{
	samplePosition = 0;
	paused = false;
	finished = false;
...
void CALLBACK ConsoleGameEngine::waveOutProcWrapper(HWAVEOUT hwo, UINT uMsg, DWORD dwInstance, DWORD_PTR dwParam1, DWORD_PTR dwParam2)
{
	((ConsoleGameEngine*)dwInstance)->waveOutProc(hwo, uMsg, dwParam1, dwParam2);
}

void ConsoleGameEngine::waveOutProc(HWAVEOUT hwo, UINT uMsg, DWORD_PTR dwParam1, DWORD_PTR dwParam2)
{
	if (uMsg == WOM_DONE)
	{
		freeBlocks++;
...
float ConsoleGameEngine::GetMixerOutput(float timeStep, int channel)
{
...
void ConsoleGameEngine::PlayAudioClip(int id, bool loop)
{
...
unsigned int ConsoleGameEngine::LoadAudioClip(std::wstring fileName)
{
...
bool ConsoleGameEngine::StartAudio(int samplesPerSec, int channels, int blockCount, int samplesPerBlock)
{
...
void ConsoleGameEngine::DestroyAudio()
{
...
float ConsoleGameEngine::OnUserSoundSample(int channel, float globalTime, float timeStep)
{
	return 0.0f;
}

float ConsoleGameEngine::OnUserSoundFilter(int channel, float globalTime, float mixedSample)
{
	return mixedSample;
}

int ConsoleGameEngine::GetVolume()
{
	return currentVolume;
}

void ConsoleGameEngine::SetVolume(float percent)
{
	if (percent < 0)
		percent = 0;
	else if (percent > 100)
...
void ConsoleGameEngine::MuteAudio()
{
	soundMuted = true;
	waveOutSetVolume(device, 0x0000);
}

void ConsoleGameEngine::UnmuteAudio()
{
	soundMuted = false;
	waveOutSetVolume(device, currentVolume);
}

void ConsoleGameEngine::PauseAudio(int id)
{
	for (auto &clip : currentlyPlayingClips)
	{
...
void ConsoleGameEngine::PauseAllAudio()
{
	for (auto &clip : currentlyPlayingClips)
	{
		clip.paused = !clip.paused;
...
void ConsoleGameEngine::RestartAudio(int id)
{
	for (auto &clip : currentlyPlayingClips)
	{
		if (clip.audioClipID == id)
...
void ConsoleGameEngine::RestartAllAudio()
{
	for (auto &clip : currentlyPlayingClips)
	{
		clip.Restart();
...
void ConsoleGameEngine::StopAudio(int id)
{
	for (auto &clip : currentlyPlayingClips)
	{
		if (clip.audioClipID == id)
...
void ConsoleGameEngine::StopAllAudio()
{
